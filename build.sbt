...
lazy val `e2e-pekko` = (projectMatrix in file("e2e-pekko"))
  ...
  .settings(
    ...
    pekkoGrpcCodeGeneratorSettings := Seq("grpc", "single_line_to_proto_string", "scala3_sources"),
    scalacOptions ++= Seq(
      "-Wconf:src=pekko-grpc/.*:silent" // Ignore warnings in classes generated by pekko-grpc
    ),
    Compile / PB.targets ++= Seq(
      (genModule("com.improving.grpc_rest_gateway.compiler.PekkoGatewayGenerator$"), Seq("scala3_sources")) ->
