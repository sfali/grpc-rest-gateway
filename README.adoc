...
class TestServiceBGatewayHandler(channel: ManagedChannel)(implicit ec: ExecutionContext)
  ...
  override protected def dispatchCall(method: HttpMethod, uri: String, body: String): Future[GeneratedMessage] = {
    ...
    if (isSupportedCall(HttpMethod.GET.name, GetGetRequestPath, methodName, path)) {
      val parameters = mergeParameters(GetGetRequestPath, queryString)
      val input = Try {
...
Following table defines how HTTP request supposed to be constructed.

[cols="1,3,3,3"]
|===
| HTTP Method | Path | HTTP request | Mapping

|GET
|/v1/messages/{message_id}/{sub.subfield}
|http://localhost:7070/v1/messages/xyz/abc
|Both `message_id` and `sub.subfield` are mapped as path variables

|GET
|/v1/messages/{message_id}
|http://localhost:7070/v1/messages/xyz?sub.subfield=abc
|`message_id` is mapped as path variable while `sub.subfield` is mapped as query parameter

|PUT
|/v1/messages/{message_id}
|http://localhost:7070/v1/messages/xyz
`body`: `{"subfield": "sub"}`
|`message_id` is mapped as path variable while `sub` is mapped as body payload

|POST
|/v1/messages
|http://localhost:7070/v1/messages
`body`: `{"message_id": "abc", "sub": {"subfield": "sub"}}`
|entire message is mapped as body payload
...
addSbtPlugin("io.github.sfali23" % "grpc-rest-gateway-code-gen" % "0.3.0")
...
Compile / PB.targets := Seq(
  ...
  com.improving.grpc_rest_gateway.gatewayGen -> -> (Compile / sourceManaged).value / "scalapb",
  com.improving.grpc_rest_gateway.swaggerGen -> -> (Compile / resourceManaged).value / "scalapb"
)

libraryDependencies += "io.github.sfali23" %% "grpc-rest-gateway-runtime" % "0.3.0"
----
