...
. https://akka.io/[Akka] based runtime library &mdash; `grpc-rest-gateway-runtime-akka`. **Coming Soon**
...
Code generation library is responsible for reading given Protobuf files and generating corresponding implementation of `GrpcGatewayHandler` based on its runtime library. There are three different plugins to generate runtime handlers, namely:

. `grpc_rest_gateway.nettyGatewayGen()` for Netty based implementation
. `grpc_rest_gateway.pekkoGatewayGen()` for Pekko based implementation
. `grpc_rest_gateway.akkaGatewayGen()` for Akka based implementation
...
libraryDependencies ++= Seq(
  ...
  "io.github.sfali23" % "grpc-rest-gateway-code-gen" % "0.8.0"
)
----
...
[source, sbt]
----
// replace grpc_rest_gateway.nettyGatewayGen with either grpc_rest_gateway.pekkoGatewayGen or grpc_rest_gateway.akkaGatewayGen
// to generate Pekko or Akka based implementations

Compile / PB.targets := Seq(
  scalapb.gen() -> (Compile / sourceManaged).value / "scalapb",
  grpc_rest_gateway.nettyGatewayGen() -> (Compile / sourceManaged).value / "scalapb",
...
Compile / PB.targets := Seq(
  ...
  grpc_rest_gateway.nettyGatewayGen(scala3Sources = true) -> (Compile / sourceManaged).value / "scalapb",
  grpc_rest_gateway.swaggerGen() -> (Compile / resourceDirectory).value / "scalapb"
)
...
libraryDependencies ++= Seq(
  "io.github.sfali23" %% "grpc-rest-gateway-runtime-netty" % "0.8.0",
...
val GrpcJava: String = scalapb.compiler.Version.grpcJavaVersion
libraryDependencies ++= Seq(
  "io.github.sfali23" %% "grpc-rest-gateway-runtime-pekko" % "0.8.0",
