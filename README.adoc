...
[ditaa, component-architecture, png]
----
+----------------------+     +----------------------+     +----------------------+
|                      |     |                      |     |                      |
...
=== Configure compiler plugin for code generation

The code-generator plugin has following options:

* `scala3Sources` &mdash; Generate Scala 3 sources &mdash; It is used for using `*` as wild card import instead of `_`.
...
* `useScala3Features` &mdash; Whether to use Scala 3 features. It is used to use `using` and `given` instead of `implicits`.
* `implementationType` &mdash; The target implementation type (Netty, Pekko, Akka)

The OpenApi specs generator has following options:

* `version` &mdash; the version of the OpenApi specification to generate, default value is `0.1.0-SNAPSHOT`.

See <<openapi-specs-configuration>> for more details.
...
== OpenAPI Specifications Configuration
...
* **`version`** &mdash; The version of the OpenAPI specification to generate. Default value is `0.1.0-SNAPSHOT`. This can be declared by setting the `version` option in the proto file. 
+
[source, proto]
----
option (grpc_rest_gateway.api.openapi_info) = {
...
----
* **`specsDirectory`** &mdash; The directory where OpenAPI specification files will be generated. This is configured at build time and runtime. The default value is `specs`.
...
==== Using a Custom Specs Directory at Build Time
...
* **`enabled`** &mdash; Enable or disable OpenAPI/Swagger UI functionality. Default: `true`
* **`specs-dir`** &mdash; The directory name where OpenAPI specification files are located in the classpath. Default: `"specs"`

Both properties can be overridden using environment variables:

* `OPENAPI_ENABLED` &mdash; Set to `true` or `false`
* `OPENAPI_SPECS_FOLDER` &mdash; Set to your custom directory name

==== Configuring a Custom Specs Directory
...
IMPORTANT: The `specs-dir` value must match the directory name where you generated the OpenAPI specs at build time (relative to the resource root).

==== Disabling OpenAPI/Swagger UI
...
* **Swagger UI Landing Page:** `http://localhost:7070`
...
==== Runtime Configuration (application.conf)
...
== Verifying Generated YAML Files

The gRPC-REST Gateway generates OpenAPI 3.1.0 YAML specifications from your protobuf definitions. This section explains how to verify that YAML files are generated correctly and contain valid OpenAPI specifications.

=== Quick Verification

To quickly verify YAML generation, use the provided verification script:

[source, shell]
----
./scripts/verify-yaml-generation.sh
----

This script will:

1. Compile the e2e projects
2. Check that YAML files exist in the expected locations
3. Validate basic YAML structure and OpenAPI fields
4. Verify proto-level configuration is applied correctly

=== Manual Verification

==== Check Generated Files

After compilation, YAML files are generated in the `resource_managed` directory:

[source, shell]
----
# For Scala 2.13
find e2e-netty/target/jvm-2.13/resource_managed/main/specs -name "*.yml"
find e2e-pekko/target/jvm-2.13/resource_managed/main/specs -name "*.yml"

# For Scala 2.12
find e2e-netty/target/jvm-2.12/resource_managed/main/specs-2.12 -name "*.yml"
find e2e-pekko/target/jvm-2.12/resource_managed/main/specs-2.12 -name "*.yml"

# For Scala 3
find e2e-netty/target/jvm-3/resource_managed/main -name "*.yml"
find e2e-pekko/target/jvm-3/resource_managed/main -name "*.yml"
----

==== View Generated YAML

To view the contents of a generated YAML file:

[source, shell]
----
# View TestServiceB.yml (Scala 2.13)
cat e2e-netty/target/jvm-2.13/classes/specs/TestServiceB.yml

# Or from Pekko module
cat e2e-pekko/target/jvm-2.13/classes/specs/TestServiceB.yml
----

The generated YAML should contain:

* `openapi: 3.1.0` - OpenAPI version
* `info` section with version, title, and description
* `paths` section with HTTP endpoints
* `components/schemas` section with message definitions
* `tags` section with service names

==== Verify Proto-Level Configuration

If you've configured OpenAPI metadata in your proto file using `openapi_info`:
...
Verify the generated YAML reflects these values:

[source, shell]
----
# Check version in generated YAML
grep "version:" e2e-netty/target/jvm-2.13/classes/specs/TestServiceB.yml
----

Expected output should show `version: 1.0.0` (not the default `0.1.0-SNAPSHOT`).

=== Automated Tests

The project includes comprehensive automated tests to validate YAML generation and content.

==== Running Validation Tests

Run the OpenAPI specification validation tests:

[source, shell]
----
# For Netty implementation
sbt "e2e-nettyJVM2_13/test:testOnly rest_gateway_test.OpenApiSpecValidationTest"

# For Pekko implementation
sbt "e2e-pekkoJVM2_13/test:testOnly rest_gateway_test.OpenApiSpecValidationTest"
...
The `OpenApiSpecValidationTest` validates:

1. **File Existence**: YAML files are generated and accessible from classpath
2. **YAML Structure**: Files are valid, parseable YAML
3. **OpenAPI Compliance**: Contains required OpenAPI 3.1.0 fields (openapi, info, paths, components)
4. **Proto-Level Configuration**: Version from `openapi_info` option is correctly applied
5. **HTTP Methods**: Only valid HTTP methods (GET, POST, PUT, DELETE, etc.) are used
6. **Responses**: Each operation has a responses section
7. **Status Codes**: Custom status codes from annotations are correctly applied
8. **Schema Components**: Message types are defined in components/schemas
9. **Tags**: Service names are included in tags section

==== Example Test Output

[source]
----
[info] OpenApiSpecValidationTest:
[info] OpenAPI YAML specifications
[info] - should be generated and accessible from classpath
[info] - should have valid YAML structure
[info] - should contain required OpenAPI 3.1.0 fields
[info] - should have correct version from proto-level configuration for TestServiceB
[info] - should contain valid HTTP methods in paths
[info] - should have responses section for each operation
[info] - should have correct status codes for TestServiceB operations
[info] - should have schema components referenced in paths
[info] - should have tags section with service names
...
==== Version Not Applied from Proto File
